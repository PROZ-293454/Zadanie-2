...
public class BoardView {
	...
	private Image getBallImage(ImageView ball, boolean isChosen) {
		System.out.println("ballimg"+ ball);
	...
	public List<ImageView> moveLineOfBalls(int x1, int y1, int dx, int dy, int lineLength, ImageMove move) {
		 ...
		 * 
		 * int x = x1 + move.getDx(); int y = y1 + move.getDy();
		 * 
		 * for (int i = -lineLengthBack; i < lineLengthForward; i++) { x = x1 + dx * i;
		 * y = y1 + dy * i;
		 * 
		 * try { ImageView newBall = moveBall(x, y, move.getDx(), move.getDy());
		 * movedBalls.add(newBall); } catch (ArrayIndexOutOfBoundsException e) {
		 * System.out.println("poza granicami"); continue; }
		 * 
		 * }
		 */
		return movedBalls;
